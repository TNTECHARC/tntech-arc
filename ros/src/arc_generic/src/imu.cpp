...
bool IMU::Open( char* port )
{
  ...
  //Set the default register to match sensor_msgs/Imu
  char* changeSetting = "$VNWRG,6,14*00";
  unsigned char check = Checksum(changeSetting+1, strlen(changeSetting)-4 );
...
//#########################################################
bool IMU::Read( sensor_msgs::Imu & result )
{ bool found = false;
...
//#########################################################
//All bytes between ($ and *) but not including
